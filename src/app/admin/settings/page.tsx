    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 p-8 mb-8">
        ...
        <div className="mb-6">
          <p className="text-sm text-neutral-600 mb-4">
            {t("admin.notificationsDesc") || "Enable push notifications to receive real-time updates for new orders and chat messages directly on this device."}
          </p>
          <NotificationToggle />
        </div>
        <div className="pt-4 border-t border-neutral-100 flex items-center justify-between">
          <p className="text-xs text-neutral-400 font-medium italic">
            {t("admin.notificationsHint") || "Make sure to 'Enable Push Notifications' above first."}
          </p>
          <button
            onClick={async () => {
              try {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 p-8 mb-8">
        <h2 className="text-xl font-bold mb-6 flex items-center border-b pb-4">
          ...
          {t("admin.whatsAppNotifications")}
        </h2>

        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-2">{t("admin.whatsAppNumber")}</label>
              <input 
                type="text" 
              ...
              />
              <p className="text-xs text-neutral-400 mt-1">{t("admin.countryCodeHint")}</p>
            </div>
            <div className="flex items-end pb-2">
              <label className="flex items-center space-x-3 cursor-pointer">
                ...
                />
                 <span className="text-sm font-bold">{t("admin.enableWhatsApp")}</span>
              </label>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-neutral-100 flex items-center justify-between">
            <p className="text-xs text-neutral-400 font-medium italic">
              {t("admin.saveApplyHint") || "Click save to apply changes before testing."}
            </p>
            <button
              onClick={async () => {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 p-8 mb-8">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {success && <span className="text-green-600 flex items-center text-sm font-medium"><CheckCircle className="w-4 h-4 mr-1" /> {t("admin.saveSettings")}</span>}
          </div>
        </form>
      </div>
    </div>
